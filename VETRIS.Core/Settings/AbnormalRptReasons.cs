...
{
    public class AbnormalRptReasons
    {
        #region Constructor
        public AbnormalRptReasons()
        {
        }
        #endregion
        ...
        public bool SearchRecords(string ConfigPath, ref DataSet ds,ref string ReturnMessage, ref string CatchMessage)
        {
            ...
            try
            {
                ...
                ds = DAL.DataHelper.ExecuteDataset(CoreCommon.CONNECTION_STRING, "settings_abnormal_rpt_reasons_fetch_brw", SqlRecordParams);
        ...
        #region SaveRecord
        public bool SaveRecord(string ConfigPath, AbnormalRptReasons[] ArrObj, ref string ReturnMessage, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            if (ValidateRecord(ArrObj, ref ReturnMessage, ref intRowID))
            {
                if (GenerateXML(ArrObj, ref CatchMessage))
                {
                    try
                    {
                        ...
                        if (CoreCommon.CONNECTION_STRING == string.Empty) CoreCommon.GetConnectionString(ConfigPath);
                        intExecReturn = DAL.DataHelper.ExecuteNonQuery(CoreCommon.CONNECTION_STRING, CommandType.StoredProcedure, "settings_abnormal_rpt_reasons_save", SqlRecordParams);
        ...
        private bool ValidateRecord(AbnormalRptReasons[] ArrObj, ref string ReturnMessage, ref int RowID)
        {
            bool bReturn = true;
            if (ArrObj.Length == 0)
            {
        ...
        #region GenerateXML
        private bool GenerateXML(AbnormalRptReasons[] ArrObj, ref string CatchMessage)
        {
            bool bReturn = false;
