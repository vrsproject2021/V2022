    ...
    public class BusinessSources
    {
        ...
        public bool SearchRecords(string ConfigPath, ref DataSet ds,ref string ReturnMessage, ref string CatchMessage)
        {
            ...
            try
            {
                ...
                ds = DAL.DataHelper.ExecuteDataset(CoreCommon.CONNECTION_STRING, "master_business_sources_fetch_brw", SqlRecordParams);
        ...
        #region SaveRecord
        public bool SaveRecord(string ConfigPath, BusinessSources[] ArrObj, ref string ReturnMessage, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            if (ValidateRecord(ArrObj, ref ReturnMessage, ref intRowID))
            {
                if (GenerateXML(ArrObj, ref CatchMessage))
                {
                    try
                    {
                        ...
                        if (CoreCommon.CONNECTION_STRING == string.Empty) CoreCommon.GetConnectionString(ConfigPath);
                        intExecReturn = DAL.DataHelper.ExecuteNonQuery(CoreCommon.CONNECTION_STRING, CommandType.StoredProcedure, "master_business_sources_save", SqlRecordParams);
        ...
        private bool ValidateRecord(BusinessSources[] ArrObj, ref string ReturnMessage, ref int RowID)
        {
            bool bReturn = true;
            if (ArrObj.Length == 0)
            {
        ...
        #region GenerateXML
        private bool GenerateXML(BusinessSources[] ArrObj, ref string CatchMessage)
        {
            bool bReturn = false;
