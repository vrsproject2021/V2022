...
namespace VETRIS.Core.Master
{
    public class Breeds
    {
        ...
        public bool SearchRecords(string ConfigPath, ref DataSet ds,ref string ReturnMessage, ref string CatchMessage)
        {
            ...
            SqlRecordParams[1] = new SqlParameter("@name", SqlDbType.NVarChar, 10); SqlRecordParams[1].Value = strName;
            SqlRecordParams[2] = new SqlParameter("@species_id", SqlDbType.NVarChar, 30); SqlRecordParams[2].Value = intSpeciesID;
            ...
            try
            {
                ...
                ds = DAL.DataHelper.ExecuteDataset(CoreCommon.CONNECTION_STRING, "master_breed_fetch_brw", SqlRecordParams);
        ...
        #region SaveRecord
        public bool SaveRecord(string ConfigPath, Breeds[] ArrObj, ref string ReturnMessage, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            if (ValidateRecord(ArrObj, ref ReturnMessage, ref intRowID))
            {
                if (GenerateXML(ArrObj, ref CatchMessage))
                {
                    try
                    {
                        ...
                        if (CoreCommon.CONNECTION_STRING == string.Empty) CoreCommon.GetConnectionString(ConfigPath);
                        intExecReturn = DAL.DataHelper.ExecuteNonQuery(CoreCommon.CONNECTION_STRING, CommandType.StoredProcedure, "master_breed_save", SqlRecordParams);
        ...
        private bool ValidateRecord(Breeds[] ArrObj, ref string ReturnMessage, ref int RowID)
        {
            bool bReturn = true;
            if (ArrObj.Length == 0)
            {
        ...
        #region GenerateXML
        private bool GenerateXML(Breeds[] ArrObj, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            try
            {
                if (ArrObj.Length > 0)
                {
                        ...
                        for (int i = 0; i < ArrObj.Length; i=i + 1)
                        {
                            ...
                            sbXML.Append("<species_id>" + ArrObj[i].SPECIES_ID + "</species_id>");
