    ...
    public class Species
    {
        ...
        public bool SearchRecords(string ConfigPath, ref DataSet ds,ref string ReturnMessage, ref string CatchMessage)
        {
            ...
            try
            {
                ...
                ds = DAL.DataHelper.ExecuteDataset(CoreCommon.CONNECTION_STRING, "master_species_fetch_brw", SqlRecordParams);
        ...
        #region SaveRecord
        public bool SaveRecord(string ConfigPath, Species[] ArrObj, ref string ReturnMessage, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            if (ValidateRecord(ArrObj, ref ReturnMessage, ref intRowID))
            {
                if (GenerateXML(ArrObj, ref CatchMessage))
                {
                    try
                    {
                        ...
                        if (CoreCommon.CONNECTION_STRING == string.Empty) CoreCommon.GetConnectionString(ConfigPath);
                        intExecReturn = DAL.DataHelper.ExecuteNonQuery(CoreCommon.CONNECTION_STRING, CommandType.StoredProcedure, "master_species_save", SqlRecordParams);
        ...
        private bool ValidateRecord(Species[] ArrObj, ref string ReturnMessage, ref int RowID)
        {
            bool bReturn = true;
            if (ArrObj.Length == 0)
            {
        ...
        #region GenerateXML
        private bool GenerateXML(Species[] ArrObj, ref string CatchMessage)
        {
            bool bReturn = false;
