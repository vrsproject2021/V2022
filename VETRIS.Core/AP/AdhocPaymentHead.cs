...
{
    public class AdhocPaymentHead
    {
        #region Constructor
        public AdhocPaymentHead()
        {
        }
        #endregion
        ...
        public bool SearchRecords(string ConfigPath, ref DataSet ds,ref string ReturnMessage, ref string CatchMessage)
        {
            ...
            SqlRecordParams[1] = new SqlParameter("@name", SqlDbType.NVarChar, 50); SqlRecordParams[1].Value = strName;
            ...
            try
            {
                ...
                ds = DAL.DataHelper.ExecuteDataset(CoreCommon.CONNECTION_STRING, "ap_adhoc_payment_head_fetch_brw", SqlRecordParams);
        ...
        #region SaveRecord
        public bool SaveRecord(string ConfigPath, AdhocPaymentHead[] ArrObj, ref string ReturnMessage, ref string CatchMessage)
        {
            bool bReturn = false;
            ...
            if (ValidateRecord(ArrObj, ref ReturnMessage, ref intRowID))
            {
                if (GenerateXML(ArrObj, ref CatchMessage))
                {
                    try
                    {
                        ...
                        if (CoreCommon.CONNECTION_STRING == string.Empty) CoreCommon.GetConnectionString(ConfigPath);
                        intExecReturn = DAL.DataHelper.ExecuteNonQuery(CoreCommon.CONNECTION_STRING, CommandType.StoredProcedure, "ap_adhoc_payment_head_save", SqlRecordParams);
        ...
        private bool ValidateRecord(AdhocPaymentHead[] ArrObj, ref string ReturnMessage, ref int RowID)
        {
            bool bReturn = true;
            if (ArrObj.Length == 0)
            {
        ...
        #region GenerateXML
        private bool GenerateXML(AdhocPaymentHead[] ArrObj, ref string CatchMessage)
        {
            bool bReturn = false;
