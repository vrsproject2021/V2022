...
function LoadRadiologist() {
    ...
    CallBackRad.callback(objhdnID.value, strType, strFilter, objhdnMenuID.value, objhdnUserID.value);
}
function btnReset_OnClick() {

    if (parent.GsRetStatus == "false") {
        strValidate = "N";
        btnBrwEditUI_Onclick('Radiologist/VRSRadAssignDlg.aspx');
    }
    else {
        parent.GsDlgConfAction = "RESUI";
        parent.GsNavURL = 'Radiologist/VRSRadAssignDlg.aspx';
        parent.PopupConfirm("028");
    }
}
function btnClose_OnClick() {
    //parent.GsNavURL = "Radiologist/VRSRadAssignBrw.aspx?uid=" + UserID + "&urid=" + UserRoleID + "&mid=" + MenuID;
    //if (parent.GsRetStatus == "false")
    //    btnDlgClose_Onclick();
    //else {
...
function btnSave_OnClick() {
    ...
    try {
        ...
        if (objrdoPrelim.checked) ArrRecords[1] = "P"; else if (objrdoFinal.checked) ArrRecords[1] = "F"; else ArrRecords[1] = "";
        ArrRecords[2] = GetRadiologist();
        ...
        VRSRadAssignDlg.SaveRecord(ArrRecords, ShowProcess);
    }
    catch (expErr) {
        HideProcess();
