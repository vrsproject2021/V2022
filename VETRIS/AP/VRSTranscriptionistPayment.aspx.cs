...
namespace VETRIS.AP
{
     [AjaxPro.AjaxNamespace("VRSTranscriptionistPayment")]
    public partial class VRSTranscriptionistPayment : System.Web.UI.Page
    {
        #region Members & Variables
        VETRIS.Core.AP.TranscriptionistPayment objCore = null;
        ...
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(VRSTranscriptionistPayment));
            SetAttributes();
            if (!CallBackPayment.CausedCallback) SetPageValue();
        }
        #endregion
        ...
        private void SetAttributes()
        {
            ...
            ddlTranscriptionist.Attributes.Add("onchange", "javascript:ddlTranscriptionist_OnChange();");
        }
        #endregion
        ...
        {
            objCore = new Core.AP.TranscriptionistPayment();
            ...
            try
            {
                ...
                if (bReturn)
                {
                    ...
                    #region Transcriptionists
                    DataRow dr2 = ds.Tables["Transcriptionist"].NewRow();
                    dr2["id"] = "00000000-0000-0000-0000-000000000000";
                    dr2["name"] = "Select One";
                    ds.Tables["Transcriptionist"].Rows.InsertAt(dr2, 0);
                    ds.Tables["Transcriptionist"].AcceptChanges();

                    ddlTranscriptionist.DataSource = ds.Tables["Transcriptionist"];
                    ddlTranscriptionist.DataValueField = "id";
                    ddlTranscriptionist.DataTextField = "name";
                    ddlTranscriptionist.DataBind();
                    #endregion
        ...
        #region CallBackPayment_Callback
        protected void CallBackPayment_Callback(object sender, ComponentArt.Web.UI.CallBackEventArgs e)
        {

            LoadDetails(e.Parameters);
            grdPayment.Width = Unit.Percentage(100);
            grdPayment.RenderControl(e.Output);
        ...
        private void LoadDetails(string[] arrRecord)
        {
            ...
            try
            {
                ...
                objCore.TRANSCRIPTIONIST_ID = new Guid(arrRecord[1]);
                ...
                if (bReturn)
                {
                    ds.Relations.Add(ds.Tables[0].Columns["transcriptionist_id"], ds.Tables[1].Columns["transcriptionist_id"]);

                    grdPayment.Levels[1].Columns[5].FormatString = objComm.DateFormat + " HH:mm";
                    grdPayment.DataSource = ds;
                    grdPayment.DataBind();
                    grdPayment.PageSize = ds.Tables["PaymentHdr"].Rows.Count;
        ...
        public string[] FetchProcessStatus(string[] ArrRecord)
        {
            ...
            try
            {
                ...
                objCore.TRANSCRIPTIONIST_ID = new Guid(ArrRecord[1].Trim());
        ...
        public string[] SaveRecord(string[] ArrRecord, string[] ArrRad)
        {
            ...
            Core.AP.TranscriptionistList[] objRad = new Core.AP.TranscriptionistList[0];
            ...
            try
            {
                ...
                objRad = new Core.AP.TranscriptionistList[(ArrRad.Length / 4)];

                #region Populate Radiologist
                for (int i = 0; i < objRad.Length; i++)
                {
                    objRad[i] = new Core.AP.TranscriptionistList();
