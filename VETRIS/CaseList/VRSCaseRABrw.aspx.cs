...
{
    public partial class VRSCaseRABrw : System.Web.UI.Page
    {
        ...
        private void SearchRecord(string[] arrRecord)
        {
            ...
            try
            {
                ...
                objCore.MODALITY = arrRecord[1].Trim();
                objCore.INSTITUTION_ID = new Guid(arrRecord[2]);
                objCore.FILTER_BY_RECEIVED_DATE = arrRecord[3].Trim();
                objCore.RECEIVED_DATE_FROM = Convert.ToDateTime(arrRecord[4]);
                objCore.RECEIVED_DATE_TILL = Convert.ToDateTime(arrRecord[5]);
                objCore.STUDY_UID = arrRecord[6].Trim();
                objCore.USER_ID = new Guid(arrRecord[7].Trim());
                objCore.MENU_ID = Convert.ToInt32(arrRecord[8].Trim());
                objCore.USER_SESSION_ID = new Guid(arrRecord[9].Trim());
                strUserRoleCode = arrRecord[10].Trim();

                bReturn = objCore.SearchRequireActionBrowserList(Server.MapPath("~"), ref ds, ref strCatchMessage);

                if (bReturn)
                {

                    grdBrw.DataSource = ds.Tables["BrowserList"];
                    grdBrw.Levels[0].Columns[4].FormatString = objComm.DateFormat + " HH:mm";
                    grdBrw.DataBind();
            ...
            if ((strUserRoleCode == "SUPP"))
            {
                grdBrw.Levels[0].Columns[1].Visible = true;
                grdBrw.Levels[0].Columns[2].Visible = true;
                grdBrw.Levels[0].Columns[8].Visible = true;
            }
            else if ((strUserRoleCode == "SYSADMIN"))
            {
                grdBrw.Levels[0].Columns[2].Visible = true;
                grdBrw.Levels[0].Columns[8].Visible = true;
            }
            else if ((strUserRoleCode == "AU"))
            {
                grdBrw.Levels[0].Columns[2].Visible = true;
            }
            else if ((strUserRoleCode == "RDL"))
            {
                grdBrw.Levels[0].Columns[2].Visible = true;
            }
        }
