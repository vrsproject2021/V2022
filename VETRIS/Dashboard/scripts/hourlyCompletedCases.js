...
function callCompletedSlotTime() {
    ...
    ArrRecords[2] = $('#tp option:selected').val();
    ArrRecords[3] = $('#ddlModality option:selected').val();
    VRSHourlyCompletedCases.SearchRecord(ArrRecords, apply_callback);
}
function apply_callback(lineData) {
    data = {
        ...
        divId: 'line_chart',
        data: lineData.value,
        xAxisTitle: 'Note : Only FINAL Cases are considered',
