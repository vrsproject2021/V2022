...
namespace VETRIS.Dashboard.HourlyCompletedCases
{
    [AjaxPro.AjaxNamespace("VRSHourlyCompletedCases")]
    public partial class VRSHourlyCompletedCases : System.Web.UI.Page
    {
        ...
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(VRSHourlyCompletedCases));
            SetPageValue();
        }
        ...
        public LineData SearchRecord(string[] arrRecord)
        {
            ...
            try
            {
                ...
                bReturn = objCore.SearchLineChartCompletedCases(Server.MapPath("~"), ref ds, ref strCatchMessage);

                if (bReturn)
                {
                    data = new LineData { labels = new List<string>(), datasets = new List<SlotData>() };
                    Regex creg = new Regex(@"\d+-\d+");
                    foreach (DataColumn c in ds.Tables["TimeSlot"].Columns)  //loop through the columns. 
                    {
                    ...
                    foreach (DataRow dr in ds.Tables["TimeSlot"].Rows)
                    {
