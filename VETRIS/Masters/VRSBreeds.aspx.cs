...
{
    [AjaxPro.AjaxNamespace("VRSBreeds")]
    public partial class VRSBreeds : System.Web.UI.Page
    {
        #region Members & Variables
        VETRIS.Core.Master.Breeds objCore = null;
        ...
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(VRSBreeds));
            SetAttributes();
        ...
        {
            objCore = new Core.Master.Breeds();
            ...
            try
            {
                ...
                if (bReturn)
                {
                    ...
                    foreach (DataRow dr in ds.Tables["Species"].Rows)
                    {
                        if (hdnSpecies.Value.Trim() != string.Empty) hdnSpecies.Value += objComm.RecordDivider;
                        hdnSpecies.Value += Convert.ToString(dr["id"]) + objComm.RecordDivider;
                        hdnSpecies.Value += Convert.ToString(dr["name"]).Trim();
                    }
                    #endregion
                }
                else
        ...
        private void LoadRecords(string[] arrParams)
        {
            ...
            try
            {
                ...
                objCore.SPECIES_ID = Convert.ToInt32(arrParams[3]);
                ...
                if (arrRecords.Length > 0)
                {
                    if (arrRecords[0].Trim() != "")
                    {
                        for (int i = 0; i < arrRecords.Length; i = i + 7)
                        {
                            ...
                            dr["species_id"] = Convert.ToInt32(arrRecords[i + 4]);
                            dr["is_active"] = arrRecords[i + 5].Trim();
                            dr["changed"] = arrRecords[i + 6].Trim();
                            dr["action"] = "";
                            dtbl.Rows.Add(dr);
                        }
        ...
        private DataTable CreateTable()
        {
            ...
            dtbl.Columns.Add("species_id", System.Type.GetType("System.Int32"));
        ...
        public string[] SaveRecord(string[] ArrRecord, string[] ArrParams)
        {
            ...
            objCore = new Core.Master.Breeds();
            int intListIndex = 0;
            Core.Master.Breeds[] objRecords = new Core.Master.Breeds[0];
            ...
            try
            {
                ...
                objRecords = new Core.Master.Breeds[(ArrRecord.Length / 6)];

                for (int i = 0; i < objRecords.Length; i++)
                {
                    objRecords[i] = new Core.Master.Breeds();
                    ...
                    objRecords[i].SPECIES_ID = Convert.ToInt32(ArrRecord[intListIndex + 4]);
                    objRecords[i].IS_ACTIVE = ArrRecord[intListIndex + 5].Trim();
