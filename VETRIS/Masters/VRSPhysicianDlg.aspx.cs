...
{
    [AjaxPro.AjaxNamespace("VRSPhysicianDlg")]
    public partial class VRSPhysicianDlg : System.Web.UI.Page
    {
        #region Members & Variables
        VETRIS.Core.Master.Physician objCore = null;
        ...
        {
            AjaxPro.Utility.RegisterTypeForAjax(typeof(VRSPhysicianDlg));
            SetAttributes();
        ...
        {
            objCore = new Core.Master.Physician();
        ...
        private void AddInstitution(string[] arrParams)
        {
            ...
            try
            {
                ...
                if (arrRecords.Length > 0)
                {
                    if (arrRecords[0].Trim() != "")
                    {
                        for (int i = 0; i < arrRecords.Length; i = i + 3)
                        {
                            ...
                            dr["physician_user_id"] = arrRecords[i + 2].Trim();
                            dr["del"] = "";
                            dtbl.Rows.Add(dr);
                        }
                ...
                drNew["physician_user_id"] = "00000000-0000-0000-0000-000000000000";
                drNew["del"] = "";
                dtbl.Rows.Add(drNew);
        ...
        public string[] SaveRecord(string[] ArrRecord, string[] ArrInst)
        {
            ...
            try
            {
                ...
                objInst = new Core.Master.InstitutionList[(ArrInst.Length)];

                for (int i = 0; i < objInst.Length; i++)
                {
                    objInst[i] = new Core.Master.InstitutionList();
                    objInst[i].ID = new Guid(ArrInst[intListIndex]);
                   // objInst[i].PHYSICIAN_USER_ID = new Guid(ArrInst[intListIndex + 1]);
