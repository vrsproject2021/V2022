...
function btnSave_OnClick() {
    ...
    try {
        ...
        VRSBreeds.SaveRecord(ArrRecords, arrParams, ShowProcess);
...
function GetRecords() {
    ...
    while (gridItem = grdBrw.get_table().getRow(itemIndex)) {
        changed = gridItem.Data[6].toString();
        if (changed == "Y") {
...
function ddlSpecies_OnChange(ID) {
    var itemIndex = 0; var gridItem; var RowId = "0";
    while (gridItem = grdBrw.get_table().getRow(itemIndex)) {
        RowId = gridItem.Data[0].toString();

        if (RowId == ID) {
            gridItem.Data[4] = document.getElementById("ddlSpecies_" + RowId).value;
            gridItem.Data[6] = "Y";
            break;
        }
        itemIndex++;
...
function ChkStatus_OnClick(ID) {
    ...
    while (gridItem = grdBrw.get_table().getRow(itemIndex)) {
        ...
        if (RowId == ID) {
            if (document.getElementById("chkAct_" + RowId).checked) gridItem.Data[4] = "Y";
